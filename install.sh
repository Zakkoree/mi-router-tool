...
#        echo -e " 请设置插件（Apps）的安装目录："
    ...
    local up_count=$(curl -kfsSL --connect-timeout 3 -m 5 https://api.counterapi.dev/v2/zakkorees-team-5185/mikit/up -H "Authorization: Bearer ut_e8iyP5XCLm1wOuvRXqva24cmaXT6SwEcHWMbrk8P" 2>/dev/null | grep -o '"up_count":[0-9]*' | awk -F':' '{print $2}')

    cat <<EOF > "$INSTALL_DIR/mikit/data/mikit_db"
config core 'mikit'
  option dnsmasq_status '1'
  option nginx_status '1'
  option docker '0'
  option up_count '${up_count:-1}'
EOF
    cat <<EOF > "$INSTALL_DIR/mikit/data/custom_script.sh"
#!/bin/sh

# 自定义自启脚本
EOF
    touch "$INSTALL_DIR/mikit/data/mikit_cron_db"
    touch "$INSTALL_DIR/mikit/data/dnsmasq_db"
    touch "$INSTALL_DIR/mikit/data/nginx_db"
